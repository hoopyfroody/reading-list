...
/**
 * @param {{owner: string, repo: string, branch: string, token: string}} settings
 * @param {import('./documents.js').Document} document
 * @returns {import('./sync.js').Transport}
 */
export function githubTransport(settings, document) {
  ...
  return {
    async read() {
      ...
      if (response.status === 404) {
        // The file does not exist yet. An empty document is a valid start.
        return { text: '', sha: null };
      }
      await assertOk(response);

      const body = await response.json();
      if (Array.isArray(body)) throw new MissingFileError(`${path} is a folder, not a file.`);
    ...
    async write(text, sha, message) {
      ...
      if (response.status === 409 || response.status === 422) {
        throw new StaleError('The file changed on the server.');
      }
      await assertOk(response);
